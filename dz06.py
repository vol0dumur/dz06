...
if __name__ == "__main__":
    ...
    with open(path.join(directory_in_str, "sort_info.txt"), 'w') as fh:
        ...
        for i in range(0, 6):
            fh.write(f"{SORTED_DIRS[i]} = {file_types_counter[i]} files\n")

        fh.write(22 * "-" + "\n")
        fh.write(f"total = {sum(file_types_counter)} files\n")
        
        fh.write("\n")
        fh.write(f"Known file types: {', '.join(known_types)}\n\n")
        fh.write(f"Unknown file types: {', '.join(unknown_types)}")
